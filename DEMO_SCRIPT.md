...
Screen action: Enter: "In the row containing 'Pending review', check the first enabled checkbox to the right of the status text, then click the green confirmation button below the table." Click Compile. Show Semantic IR, locator candidates, generated Playwright, response model `gpt-5.6-sol`, and token diagnostics.
...
Screen action: Click Run B. A separate Chromium window opens, visibly checks the
Pending review checkbox, clicks confirmation, and holds the completed result for
four seconds. Return to Studio and show Runtime LLM calls: 0.

Validated local result: both variants pass every step with
`openAIRequests: 0`. The tracked workflow was generated by one real GPT-5.6
compile call and selected both locators at 96% confidence.
